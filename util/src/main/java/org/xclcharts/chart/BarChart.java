...
public class BarChart extends AxesChart {
		...
		for (int i = 0; i < tickCount; i++) {
			switch (pos) {
				...
				case VERTICAL_CENTER:
					...
					// 分类
					mLstCateTick.add(new PlotAxisTick(currentX, currentY,
							categoryAxis.getDataSet().get(i), labelX, labelY,
							showTicks));
					break;
				case TOP: // X
				case BOTTOM:
				case HORIZONTAL_CENTER:
					...
					mLstCateTick.add(new PlotAxisTick(currentX, currentY2, dataSet
							.get(i), labelX, labelY, showTicks));
	...
	protected boolean renderHorizontalBar(Canvas canvas) {
		...
		for (int i = 0; i < barNumber; i++) {
			...
			for (int j = 0; j < vSize; j++) {
				bv = barValues.get(j);
				setBarDataColor(mFlatBar.getBarPaint(), barDataColor, j,
						bd.getColor());

				currLabelY = sub(barInitY, mul((j + 1), YSteps));

				if (XEnum.BarCenterStyle.SPACE == mBarCenterStyle) {
					drawBarButtomY = add(add(currLabelY, div(YSteps, 2)),
							labelBarUseHeight / 2);
				} else {
					drawBarButtomY = add(currLabelY, labelBarUseHeight / 2);
				}
				drawBarButtomY = sub(drawBarButtomY,
						add(barHeight, barInnerMargin) * currNumber);
				...
				paint.setTextSize(DensityUtil.px2dip(mContext, 50));
				paint.setTypeface(Typeface.SERIF);
				canvas.drawText(name ,  0, (barBottom + barTop) / 2, paint);
				...
				// 在柱形的顶端显示上柱形的批注形状
				drawAnchor(this.mAnchorSet, i, j, canvas, labelLeftX,
						labelLeftY);

				// 柱形顶端标识
				if (!mEqualAxisMin
						&& Double.compare(dataAxis.getAxisMin(), bv) == 0) {
				} else {
					mFlatBar.renderBarItemLabel(label, labelLeftX, labelLeftY,
							canvas);
				}

				// 显示焦点框
				drawFocusRect(canvas, i, j, barLeft, barTop, barRight,
						barBottom);
			}
			currNumber++;
		}
		...
		// 画横向柱形图，竖向的定制线
		if (null != mCustomLine) {
			mCustomLine.setHorizontalPlot(dataAxis, plotArea,
					this.getAxisScreenWidth());
	...
	public float getHPValPosition(double bv) {
		...
		float valuePostion = mul(getPlotScreenWidth(),
				div((float) (vaxlen), dataAxis.getAxisRange()));
	...
	protected float getAxisXPos(XEnum.AxisLocation location) {
		if (XEnum.Direction.HORIZONTAL == mDirection
				&& dataAxis.getAxisStdStatus()
				&& categoryAxis.getAxisBuildStdStatus()) {
	...
	protected void drawClipCategoryAxisLine(Canvas canvas) {
		if (XEnum.Direction.VERTICAL == mDirection
			...
			categoryAxis.renderAxis(canvas, plotArea.getLeft(), y,
					plotArea.getRight(), y);
		} else if (XEnum.Direction.HORIZONTAL == mDirection
				&& dataAxis.getAxisStdStatus()
				&& categoryAxis.getAxisBuildStdStatus()) {
			float x = getHPDataAxisStdX();
			categoryAxis.renderAxis(canvas, x, plotArea.getTop(), x,
					plotArea.getBottom());
		} else {
	...
	protected boolean renderVerticalBar(Canvas canvas) {
		...
		for (int i = 0; i < size; i++) {
			...
			for (int j = 0; j < countChild; j++) {
				...
				drawBarStartX = add(drawBarStartX,
						add(barWidth, barInnerMargin) * currNumber);
	 ...
	 *            默认的柱形颜色
	 */
	protected void setBarDataColor(Paint paint, List<Integer> lstDataColor,
								   int currNumber, int defaultColor) {
